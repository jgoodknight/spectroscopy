...
omega_c = mySpace.unitHandler.energyUnitsFromWavenumbers(0.0) #joel-energy-units 
...
delta_10x = mySpace.unitHandler.energyUnitsFromWavenumbers(100) #joel-distance-units 
delta_10y = 0
...
pulse_width_stdev = 10.0 / (2.0 *np.sqrt(2.0 * np.log(2.0))) * 1.0E-15 #femtoseconds
print "Monomer pulse stdev", pulse_width_stdev, "seconds"
...
xOscillatorExcited = NuclearOperator.harmonicOscillator(mySpace, 
                                                         omega = omega_10x, 
                                                         center = delta_10x, 
                                                         energyOffset = E_00) #make this energy offset 0 and give it all to the other
...
groundStateNuclearHamiltonian = NuclearOperator.nuclearHamiltonian(mySpace, listOfOneDimensionalHamiltonians = [xOscillatorGround, yOscillatorGround] )

excitedStateNuclearHamiltonian = NuclearOperator.nuclearHamiltonian(mySpace, listOfOneDimensionalHamiltonians = [xOscillatorExcited, yOscillatorExcited] )
...
maxFrequency =  mySpace.unitHandler.energyUnitsFromWavenumbers(3000.0)
frequencyResolution =  mySpace.unitHandler.energyUnitsFromWavenumbers(2.0E-1)
